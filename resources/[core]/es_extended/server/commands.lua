...
ESX.RegisterCommand(
    ...
    "admin",
    function(xPlayer, args, showError)
        if not ESX.DoesJob2Exist(args.job2, args.grade2) then
            return showError(TranslateCap("command_setjob_invalid"))
        end
...
ESX.RegisterCommand(
    ...
    "admin",
    function(xPlayer, args, showError)
        if not ESX.DoesJob3Exist(args.job3, args.grade3) then
            return showError(TranslateCap("command_setjob_invalid"))
        end

        args.playerId.setJob3(args.job3, args.grade3)
        if Config.AdminLogging then
            ESX.DiscordLogFields("UserActions", "Set Job3 /setjob3 Triggered!", "pink", {
                { name = "Player", value = xPlayer and xPlayer.name or "Server Console", inline = true },
                { name = "ID", value = xPlayer and xPlayer.source or "Unknown ID", inline = true },
                { name = "Target", value = args.playerId.name, inline = true },
...
ESX.RegisterCommand("job2", { "user", "admin" }, function(xPlayer, _, _)
    print(("%s, your job2 is: ^5%s^0 - ^5%s^0"):format(xPlayer.getName(), xPlayer.getJob2().name, xPlayer.getJob2().grade_label))
end, false)

ESX.RegisterCommand("job3", { "user", "admin" }, function(xPlayer, _, _)
    print(("%s, your job3 is: ^5%s^0 - ^5%s^0"):format(xPlayer.getName(), xPlayer.getJob3().name, xPlayer.getJob3().grade_label))
end, false)
...
ESX.RegisterCommand("info2", { "user", "admin" }, function(xPlayer)
    local job2 = xPlayer.getJob2().name
    print(("^2ID: ^5%s^0 | ^2Name: ^5%s^0 | ^2Group: ^5%s^0 | ^2Job2: ^5%s^0"):format(xPlayer.source, xPlayer.getName(), xPlayer.getGroup(), job2))
end, false)

ESX.RegisterCommand("info3", { "user", "admin" }, function(xPlayer)
    local job3 = xPlayer.getJob3().name
    print(("^2ID: ^5%s^0 | ^2Name: ^5%s^0 | ^2Group: ^5%s^0 | ^2Job3: ^5%s^0"):format(xPlayer.source, xPlayer.getName(), xPlayer.getGroup(), job3))
end, false)
